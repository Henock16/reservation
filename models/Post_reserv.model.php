...
if($nbpages>1){
    ...
    $pagination.='<li class="'.(($page==1)?'disabled':'bouton').'">
                    <a href="javascript:loadreservations('.(($page>1)?($page-1):1).')" aria-label="Page précédante">
    ...
    for ($i =$min ; $i <= $max; $i++)
        $pagination.='<li class="'.(($i==$page)?'active':'bouton').'" aria-label="Page '.$i.'">
                        <a href="javascript:loadreservations('.$i.')">'.$i.'</a>
                      </li>';
    
    $pagination.='<li class="'.(($page==$nbpages)?'disabled':'bouton').'">
                    <a href="javascript:loadreservations('.(($page<$nbpages)?($page+1):$nbpages).')" aria-label="Page suivante">
...
$messages='<table class="table table-sm" id="tab_logic" style="font-size:12px;margin-top:-15px;margin-bottom:0px;padding-bottom:0px;">
             <thead>
                    <tr>
                    <th style="text-align:center;;background-color:lightblue;">Date r&eacute;servation</th>
                    <th style="text-align:center;;background-color:lightblue;">Plage</th>
                    <th style="text-align:center;;background-color:lightblue;">Pont bascule / Station d\'empotage</th>
...
while ($donnees = $result->fetch()){
        ...
        $messages .= '<tr>                                                   
                        ...
                        <td style="text-align:center;" bgcolor="lightgray">
                            ...
                            <li class="bouton">
                            <a href="javascript:delreservations('.$donnees['ID'].','.$page.')">Annuler</a>
                            </li>
                            </ul>
                        </td>
                       </tr>';
