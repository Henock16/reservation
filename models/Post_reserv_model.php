...
<<<<<<< HEAD
	include('../functions/Log_function.php');
	include('../functions/Mail_function.php');
...
<<<<<<< HEAD
		$idreserv=$bdd->lastInsertId();

		logger($bdd,$_SESSION["ID_UTIL"], 1, 1,$idreserv,"");
=======
>>>>>>> 686f7821902170a957ef7e43867a07ae1e40e643
...
<<<<<<< HEAD
        $tab[2]='';
        $tab[3]='Réservation numéro <strong>'.$idreserv.'</strong> ';							
        $tab[3].='<br/>effectuée avec succès !';							
=======
        $tab[2]='FELICITATIONS';
...
>>>>>>> 686f7821902170a957ef7e43867a07ae1e40e643
        ...
        if($_SESSION['VILLE']==1)
            {
<<<<<<< HEAD
            envoimail( $email,$sj,$msg,'','');	
            envoimail( $email1,$sj,$msg,'','');	
            envoimail( $email2,$sj,$msg,'','');
            }					
        elseif($_SESSION['VILLE']==2)
            {
            envoimail( $email3,$sj,$msg,'','');
            envoimail( $email4,$sj,$msg,'','');	
            envoimail( $email5,$sj,$msg,'','');
            }		
...
<<<<<<< HEAD
	$datereserv= datesitetoserver($_POST["date_reserv"]); //j'ai change la date de francais en anglais

	$jour=date("Y-m-d");
	$heur= date("H:m:s");
	$timeis=(($_SESSION["VILLE"]==1)?$heure_ab:(($_SESSION["VILLE"]==2)?$heure_sp:'16:00:00'));

	if(isaffectable($bdd,$datereserv,$_POST["radio"],$jour,$heur,$timeis)){	//Si la reservation peut avoir une affectation
		...
		if( $lign_count == 0){ // Si la réservation n'existe pas encore dans la base de données, on l'insère
			...
			$res=$bdd->prepare("SELECT * FROM User WHERE IDENTIFIANT=:id ");
			$res->bindParam(':id', $_SESSION["ID_UTIL"], PDO::PARAM_INT);
			$res->execute();
			$don = $res->fetch();
			...
			if($working_time){
				...
				$mssg="<p style=\"color:red;\">
						La réservation de cette plage horaire est suspendue pour impayés de la prestation de réservation des inspecteurs de poids. 
						Prière de prendre attache immédiatement avec <b>Mme Clarisse Mariam BAKAYOKO Epse CISSE</b> 
						(<b>27 20 33 16 00 Poste 758</b> / <b>07 09 85 64 28</b> ou par email à l'adresse <b>mariambakayoko@cci.ci</b>).</p>";
				$bttn='OK';
				$bout='btn btn-danger';
			}else{
				$conf=0;
				...
				$mssg="<p style=\"color:red;\">
						...
						Pour toute commande ou réservation de peseurs jurés en dehors des horaires de travail en vigueur à la CCI-CI (<b>les nuits et les jours non ouvrés ou fériés</b>), un montant complémentaire de <b>".number_format($tarif,0,""," ")." FCFA HT</b> sera exigé.<br/>
						...
						<p style=\"color:red;\">
						Cette prestation vous sera facturée à <b>".number_format($tarif,0,""," ")." FCFA HT</b>
						</p>";
				$bttn='Valider la réservation';
				$bout='btn btn-danger';
			}
...
<<<<<<< HEAD
	...
	while ($donnees = $result->fetch()){
			...
			$messages .= '<tr>                                                   
							...
							<input type="text" style="text-align:center; height: 30px;" class="form-control" name="" value="'.dateservertosite($donnees['DATE_RESERVATION']).'" disabled="disabled">
							</td>
...
<<<<<<< HEAD
	...
	logger($bdd,$_SESSION["ID_UTIL"], 4, 1,$_POST['id'],"");
	...
	$don = $res->fetch();
	$sj='Annulation';
	$msg="<span>La réservation du site ".$don["LIBELLE"]." pour le ".dateservertosite($don['DATE_RESERVATION'])." a été annulée par ".$_SESSION["STRUCTURE"]."</span>
